...
[Jetstream](https://docs.bsky.app/blog/jetstream) solved the bandwidth problem. Unlike Tap, it delivers only the collections you subscribe to — no local filtering of the full firehose. The tradeoff: no built-in database, and no backfilling. For docs.surf, which only needs the latest 100 posts, that's fine.
