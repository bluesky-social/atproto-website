...
The account management page and the OAuth sign-in screens are the parts of your PDS that your users actually look at. Until now, everyone's looked identical: the same neutral palette, the same generic service name, nothing to distinguish one host from another at the moment a user is being asked to type their password.

Now the sign-in screens, the consent screens, and the account management interface now take your name, your logo, your colors, and your links!

All of this is configured with environment variables. If you're running the reference PDS with the standard Docker deployment, you can start experimenting in your `pds.env` and restart. Start with a name and logo:
...
From there you can go as deep as you like. There are `PDS_ERROR_COLOR`, `PDS_WARNING_COLOR`, `PDS_INFO_COLOR`, and `PDS_SUCCESS_COLOR` for the semantic states, and `PDS_LIGHT_COLOR` and `PDS_DARK_COLOR` to set the two ends of the neutral range.

Many of these are optional. If you specify `PDS_PRIMARY_COLOR` but not `PDS_PRIMARY_COLOR_CONTRAST`, the interface picks whichever of your light or dark colors has the better contrast ratio against it. The defaults are accessible, and you can deviate from there.
...
You can also now set a background image(!) for the auth screens, which sits behind the sign-in card:
...
We're also planning on allowing the PDS to report its maximum blob size in the future, so clients can split or reject uploads if desired.
...
All in all, this is a big release for the reference PDS, and knocks out all remaining items on our summer PDS to-do list. If you're running the reference PDS with the standard Docker deployment, it'll automatically update to the newest release. Try it out and let us know what you think!