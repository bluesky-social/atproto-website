...
The new `network.bsky.jetstream.*` methods that power Replay — `planSnapshot`, `listSegments`, `getSegment`, and `getBlock` — are now browsable with full request and response schemas, and the reference now documents Jetstream's WebSocket endpoint too, so the entire Jetstream v2 surface lives in one place.
