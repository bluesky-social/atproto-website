...
- `key` (string, required): specifies the [record key](/specs/record-key) type
...
Leap seconds are not supported. The seconds value must be between `00` and `59`. Datetime strings with a seconds value of `60` or higher are invalid.
...
# smallest and largest datetimes representable
# beware that these don't normalize into valid datetimes
