...
- `resource` (string, required): indicates what account resource is being granted access to
...
- public repository write permissions (`"resource": "repo"`)
    - `collection` (array of unique strings, required): indicates record types. Must include at least one element (not empty array). Wildcard (`*`) or partial-NSID glob patterns (`com.example.*`) are **NOT** allowed.
    - `action` (array of unique strings, optional): limits set of record operations. Values must be one of `create`, `update`, `delete`, case-sensitive.
- `rpc`: remote API calls via proxying or service token generation
    - `lxm` (array of strings, required): allowed XRPC endpoints, as NSIDs. Must include at least one element (not empty array). Wildcard (`*`) or partial-NSID glob patterns (`com.example.*`) are **NOT** allowed.
    - `aud` (string, semi-required): remote service, identified by DID service reference (DID followed by required service type fragment, eg `did:web:api.example.com#srvtype`). Supports wildcard (`*`). Required unless `inheritAud` is true. Cannot be `*` when `lxm` is `*`.
...
`account` and `identity` permissions **cannot** be granted through permission sets and **must** be ignored. This is due to the fact that permission sets are used to conceal multiple app specific permission under a single item in the consent screen. Because of this only resources (record types and lexicon methods) under the authority of the Lexicon document author can be listed in permission sets.
