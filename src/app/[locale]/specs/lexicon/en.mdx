...
- `blob`: media file upload permissions
    - `accept` (array of strings, required): mimetypes or partial mimetype glob patterns (`*/*` or `text/*` for example). Same syntax as the `blob` lexicon type.
...
- `rpc`: remote API calls via proxying or service token generation
    ...
    - `audience` (string, semi-optional): remote service, identified by DID service reference (DID followed by required service type fragment, eg `did:web:api.example.com#srvtype`). Supports wildcard (`*`). Required unless `inheritAud` is true.
    - `inheritAud` (boolean, optional): if true, `audience` does not need to be defined.
