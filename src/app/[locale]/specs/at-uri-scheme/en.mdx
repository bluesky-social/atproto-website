...
The **authority** section is required, must be normalized, and if a DID must be one of the "blessed" DID methods. If the **authority** section is a DID, it must be [percent-encoded](https://en.wikipedia.org/wiki/Percent-encoding) if it is not already. The **authority** section may be displayed without percent-encoding.

The optional **collection** part of the path must be a normalized [NSID](./nsid). The optional **rkey** part of the path must be a valid [Record Key](./record-key).
...
- Handle as authority part: supported
- DID as authority part: only supported if [percent-encoded](https://en.wikipedia.org/wiki/Percent-encoding), must not be double-encoded
...
- Authority as DID: in normalized form, and no duplicate percent-encoding. For example, if the DID is already percent-encoded, don't re-encode the percent signs.
