...
<Note>
AT URIs which use handles as the authority are not durable. DID-based URIs are recommended instead. If a user changes their handle, any AT URIs using that handle will become invalid and could potentially point to a record in another repo if the handle is reused.
</Note>
