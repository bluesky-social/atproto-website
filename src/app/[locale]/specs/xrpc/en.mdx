...
The issuer field (`iss`) is the DID of the account sending the request. If an issuer key identifier (`kid`) field is provided, it indicates a specific public key from the account's DID document in the `verificationMethod` array, defaulting to `atproto`. This field authenticates the sender of the request. Receiving services must only accept specific key types (verification method identifiers), to maintain security separation between registered keys. This usually means accepting only `atproto`, unless there is a specific application-specific or service-specific additional key type expected (eg, `atproto_label`).
