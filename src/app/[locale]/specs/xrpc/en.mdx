...
- `iss` body field (string, required): account DID ("issuer") that the request is being sent on behalf of. May optionally include a fragment name identifying the key ("verification method") used to sign the JWT, which defaults to `#atproto`. See [DID syntax specification](/specs/did) for syntax details, and below for semantics.
...
The issuer field (`iss`) is the DID of the account sending the request. If a fragment name is provided, it indicates a specific public key from the account's DID document in the `verificationMethod` array. If no fragment is provided, the default atproto signing key (`#atproto`) is used. This field authenticates the sender of the request. Receiving services must only accept specific verification method types, to maintain security separation between registered keys. This usually means accepting only `#atproto`, unless there is a specific application-specific or service-specific additional key type expected.

<Note>
Until Spring 2026, the issuer (`iss`) optional fragment identified a service endpoint, not a public key (verification method). This feature was not frequently used, and the specification has been changed.
</Note>
