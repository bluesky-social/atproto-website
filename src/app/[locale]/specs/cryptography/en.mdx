...
A common pattern when signing data in atproto is to encode the data in a strict CBOR format ([DRISL](https://dasl.ing/drisl.html)), hash the CBOR bytes with SHA-256, yielding raw bytes (not a hex-encoded string), and then sign the hash bytes.
