...
The IPFS CID specification is very flexible, and supports a wide variety of hash types, a field indicating the type of content being linked to, and various string encoding options. These features are valuable to allow evolution of the repo format over time, but to maximize interoperability among implementations, only a specific "blessed" set of CID types are allowed. This "blessed" format aligns with the [DASL CID](https://dasl.ing/cid.html) specification.
...
- CIDv1
- Multibase: binary serialization within DRISL CBOR (or `base32` for JSON mappings)
- Multicodec: `dag-cbor` (0x71) for DRISL CBOR data
