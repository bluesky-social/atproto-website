...
- The client assertion JWT must include an `jti` claim (JWT ID) with a unique value to prevent replays. Authorization Servers must ensure uniqueness of `jti` values.
