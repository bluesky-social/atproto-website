...
The auth flow page will display all the relevent permissions being granted. This is true both for apps using granular permissions, and for those using the [transitional](#authorization-scopes) OAuth scopes.
...
The intention is to support flexible scopes based on Lexicon namespaces (NSIDs) so that clients can be given access only to the specific content and API endpoints they need access to. **Work on permission sets is in progress**. Currently, the `atproto` profile of OAuth defines `transition` scopes which align with the permissions granted under the original "session token" auth system:
...
Granular permissions have so far only been implemented on the `bsky.social` hosting service. The ergonomics of OAuth scopes in AT Protocol are still evolving. The current implementation is documented in the following section.

### Scope String Details

Permission scope strings are case-sensitive and use a subset of printable non-whitespace ASCII characters. They include a resource name, followed by an optional "positional" part separated by a colon (:), followed by optional parameters starting with a question mark (?): for example, `rpc:app.bsky.actor.getProfile?aud=did:web:api.bsky.app#bsky_appview` is used to fetch the user's own profile. The `atproto` scope is required for a minimal login flow that supports signing in with Bluesky but grants no additional permissions (authentication only, no authorization).

The positional substring following the question mark must be a valid URL-encoded query string, and some implmentations may support non-encoded characters. In general, this means it may contain any characters except for a question mark, including additional colon characters, hash (#), period, etc. and support being percent encoded. Parameters are separated by ampersands (&), have name and value parts separated by equals (=), and support percent-encoding of reserved characters. Array values can be represented by repeated parameters with the same name part.

**Repo Permissions**: the `collection` parameter can be positional or an array, and specifies the NSIDs of record types. Wildcard `collection` is allowed and covers all record types; partial wildcard ("glob patterns") is not allowed. The `action` parameter can be an array of values, and includes `create`, `update`, and `delete`. The default `action` (if not specified) is all three actions.

```
# Full permission (Create, Update, Delete) on the Bluesky profile record type
repo:app.bsky.actor.profile

# Same as above, using the named parameter instead of a positional parameter
repo?collection=app.bsky.actor.profile

# Same as above, with every possible action being explicit
...
**RPC Permissions**: the `lxm` (lexicon method) parameter is required, may be positional, has NSID syntax, and represents one (or more) API endpoints. The `aud` (audience) parameter is required, has DID-and-fragment syntax, represents the target service, and must be either a single value or wildcard. Either `lxm` or `aud` may be wildcard, but both can not be wildcard at the same time. Any `rpc` scope string containing a wildcard in both `lxm` and `aud` positions must be rejected.
...
**Blob Permissions**: the `accept` parameter is required, and may be positional. It is a MIME type pattern, with partial wildcard ("glob pattern") supported (`*/*` and `text/*` blob patterns are allowed, prefix-only like `*/html` is not).
...
**Account Permissions**: the `attr` parameter is required, and may be positional. The `action` parameter is optional, can be `read` or `manage` (which includes read/write permissions), and defaults to `read`. The initial supported account attributes are `email` (account email address) and `repo` (for importing entire data repositories, as part of account migration). `status` (account hosting status) can be declared, but is not currently implemented anywhere.
...
**Identity Permissions**: the `attr` parameter is required, may be positional, and may be a wildcard (`*`). The `action` parameter is optional, defaults to `manage`, and can also be set to `submit`. The initial `attr` parameters are `handle` (for updating just the account handle), or `*` for full DID document control.

```
# Manage account handle
identity:handle

# Full control over DID document, eg for account migration
identity:*
```

<Note>
Note: An earlier version of these examples showed scope `identity:*?action=submit` (allowing submission of pre-signed updates). This scope does not exist in the current implementation, but some equivalent and/or more granular `identity:` scopes may be introduced in the future.
</Note>
...
- for "untrusted" public clients, overall session lifetime and the lifetime of individual refresh tokens should both be limited to 2 weeks.
