...
Clients which are only using atproto OAuth for account authentication (without authorization to access PDS resources) should request minimal scopes (see "Authorization Scopes" section), but still need to implement most of the authorization flow. In particular, it is critical that they check the `sub` field in a token response to verify the account identity (this is an atproto-specific detail).
...
In the Authorization Request, the `redirect_uri` must match one of those supplied (or a default). Hostname and path components must match, but port numbers are not matched.

Some metadata fields can be configured via query parameter in the `client_id` URL (with appropriate URL encoding):

- `redirect_uri` (string, multiple query parameters allowed, optional): allows declaring a local redirect/callback URL. The scheme must be `http`, and the hostname must be either `localhost` or a loopback IP literal (`127.0.0.1` or `[::1]`). Path components are matched, but port numbers are ignored. The default values (if none are supplied) are `http://localhost/`, `http://127.0.0.1/`, and `http://[::1]/`.
...
<Note>
[RFC 8252](https://datatracker.ietf.org/doc/html/rfc8252#section-8.3) recommends loopback IP literals over `localhost` for native apps binding a local port listener, out of concern that `localhost` could resolve to a non-loopback address. Allowing `localhost` redirect URIs does not add meaningful risk here: an application under development is already served from and browsed at the `localhost` origin, so the OAuth callback depends on `localhost` name resolution no more than the application itself does. Mainstream browsers treat `http://localhost` as a [secure context](https://w3c.github.io/webappsec-secure-contexts/#localhost) only on the condition that `localhost` names are guaranteed to resolve to the loopback interface. Because PAR, PKCE, and DPoP are all mandatory in this profile, an intercepted authorization code cannot be redeemed: the token request requires both the PKCE verifier and the DPoP key which were bound to the session when the authorization flow started. Allowing `localhost` redirect URIs avoids a common development failure mode where OAuth session state is stored under the `localhost` origin in the browser, but a loopback IP `redirect_uri` returns the user to a different origin which cannot access that state.
</Note>
...
The special `atproto` scope is required for all atproto OAuth sessions. The semantics are similar to the `openid` scope: inclusion of it confirms that the client is using the atproto profile of OAuth and will comply with all the requirements laid out in this specification. No access to any atproto-specific PDS resources will be granted without this scope included.

Authorization Servers may support other profiles of OAuth if the client does not include the `atproto` scope. For example, an Authorization Server might function as both an atproto PDS/entryway, and support other protocols/standards at the same time.
...
The [Permissions Specification](/specs/permission) describes the syntax and semantics of atproto-specific resource permissions.

### Transitional Scopes

The atproto OAuth profile and Permissions system were not specified in the early days of atproto. To make migration of that software easier, the atproto profile of OAuth defines a small set of "transitional" scopes which align with the permissions granted under the original password-based auth system:
...
The goal is to deprecate and eventually remove these transitional scopes.
...
There is a special exception for the localhost development workflow to use `http://localhost`, `http://127.0.0.1`, or `http://[::1]` URLs, with matching rules described in the "Localhost Client Development" section. These clients use web URLs, but have `application_type` set to `native` in the generated client metadata.
...
Access tokens are used to authorize client requests to the account's PDS ("Resource Server"). From the standpoint of the client they are opaque tokens. Depending on the PDS implementation, it may or may not be possible to revoke individual access tokens in the event of a compromise, so they must be restricted to a relatively short lifetime.
