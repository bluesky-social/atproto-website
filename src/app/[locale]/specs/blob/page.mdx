...
The same blob can be referenced by multiple records in the same repository. Re-uploaded a blob which has already been stored and referenced results in no change the the existing blobs or records.
