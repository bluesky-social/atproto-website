...
Handles should generally not be truncated to local context. For example, the handle `@robin.example.com` should not be displayed as `@robin`, even in the local context of an `example.com` service.
