...
大規模な分散型バックエンドをセットアップしたので、ATプロトコルでアプリケーションが実際にどのように動作するかを図にしてみましょう。
...
順調なスタートです。ユーザーのリポジトリでJSONドキュメントを読み書きできます。他のアプリ（プロファイルなど）から既にデータがある場合は、そのデータも読み取ることができます。シングルプレイヤーアプリを作成している場合は、すでに完了しています。
