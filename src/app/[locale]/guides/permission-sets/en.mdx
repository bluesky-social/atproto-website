...
This page explains how to **define** permission sets for custom Lexicons. If you just want to **use** existing permission scopes in your OAuth flow, see [Permission Sets](/guides/scopes).
...
If you're building an app with **custom Lexicons**, you should define permission sets for them. This gives users clear descriptions of what your app can do, rather than a list of opaque namespace IDs. See [Permission Set design](#permission-set-design) below.
...
## Lexicons and permissions

Lexicons are authored in JSON — you can find examples in the [Lexicon Guide](/guides/lexicon). Each Lexicon becomes a permission scope: `app.bsky.feed.post` is simultaneously an AT record format, a web endpoint, and a granular permission.

Lexicon schemas are published publicly as records in atproto repositories. Refer to [Lexicon Publication and Resolution](/specs/lexicon#lexicon-publication-and-resolution) for technical details on publishing.
...
The `title` and `details` fields are descriptive, and will be displayed as part of the OAuth flow; see the [OAuth flow examples](#oauth-flow-examples) section for an example. Internationalization of these descriptive fields is supported — refer to [the permission sets spec](/specs/permission#permission-sets).
...
Permission sets are Lexicons, and are published using the same tooling. See [Publishing Lexicons](/guides/publishing-lexicons) for instructions on using `goat` to create, lint, and publish your permission set Lexicons.

After publishing, you can verify that your permission set is live with `goat lex resolve`:
