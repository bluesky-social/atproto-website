...
Refer to the [for users](#for-users) section for how these requests appear in the OAuth flow.
...
When your app initiates an OAuth flow with an ATProto identity provider, it can request specific permission sets by including them in the `scope` parameter of the authorization request. This can be supplied as a *space-separated list* in the `oauth-client-metadata.json` file when registering your OAuth client:
...
Early AT permission scopes would appear to grant permissions to create, update, or delete any of a user's AT records, including posts from other applications. In general, this should not be necessary — you should try to only request enough permissions to cover the features your app needs. This helps build trust with your users, and minimizes friction during the authorization process.

From a user experience perspective, it's best to keep the number of requested permission sets low. Each additional permission set adds more information to the authorization dialog, which can lead to decision fatigue. It is possible to allow users to grant only a subset of requested permissions, but this can lead to edge cases and is rarely worth it from a development perspective.

### Rolling out changes

When you update your app to use new permission sets, it's important to coordinate the software release with the permission-set updates. If you add new permission sets that require user approval, users may need to re-authorize your app to grant these new permissions.

## For Lexicon designers

Lexicons are authored in JSON — you can find examples in the [Lexicon Guide](/guides/lexicon). Atmosphere HTTP API methods each scope and implement a particular set of Lexicons, so Lexicons must include valid query parameters, request bodies, and response bodies as appropriate. Each Lexicon, in turn, becomes a permission scope — in other words, `app.bsky.feed.post` is an AT record format, a web endpoint, and a granular permission all at once.
...
The `title` and `details` fields are descriptive, and will be displayed as part of the OAuth flow; see the [for users](#for-users) section for an example. Internationalization of these descriptive fields is supported — refer to [the permission sets spec](/specs/permission#permission-sets).
