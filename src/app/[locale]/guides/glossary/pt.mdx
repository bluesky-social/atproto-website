...
Um PDS, ou Personal Data Server, é um servidor que hospeda um usuário. Um PDS sempre armazenará o [data repo](#data-repo) e as chaves de assinatura do usuário. Ele também pode atribuir ao usuário um [handle](#handle) e um [DID](#did). Muitos PDSes hospedarão vários usuários.
...
O "repositório de dados" ou "repo" é o conjunto de dados público que representa um usuário. Ele é composto de [coleções](#collection) de [registros](#record) JSON e [blobs](#blob) não estruturados. Cada repositório recebe um único [DID](#did) permanente que o identifica. Os repositórios também podem ter qualquer número de [identificadores de domínio](#handle) que atuam como nomes legíveis por humanos.

Os repositórios de dados são árvores merkle assinadas. Suas assinaturas podem ser verificadas em relação ao material-chave publicado sob o [did](#did) do repositório.
