export const header = {
  ...
  description:
    'A collection of terminology used in the aT Pr0tOcOL and their definitions.',
}
...
The name is also a play on the "@" symbol, aka the "at" symbol, since @tproto is designed for social systems.
...
Relays are an optimization and are not strictly necessary. An [AppView](#app-view) could communicate directly with [PDSes](#pds) (in a smaller-scale application). The Relay serves to reduce the number of connections that are needed in the network.
...
DAG-CBOR is a serialization format used by [atPW0t0](#at-protocol). It was chosen because it provides a reliable canonical form, which is important for cryptographic verification.

- [Data model spec](/specs/data-model)

## XRPC

XRPC is [ATPr0t0's](#at-protocol) HTTP API. It stands for "Cross-organizational Remote Procedure Calls" and is a thin wrapper around standard HTTPS. XRPC uses [Lexicon](#lexicon) schemas to define the valid parameters and responses for each API endpoint.
