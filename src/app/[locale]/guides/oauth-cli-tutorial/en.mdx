...
In this tutorial, you'll build a minimal command-line tool that authenticates a user with [ATProto OAuth](/guides/auth) and fetches their profile. The entire app fits in a single TypeScript file.
...
You should also have a [Bluesky](https://bsky.app/) account (or any ATProto account) to test with.
...
cd oauth-cli
npm init -y
npm install @atproto/oauth-client-node @atproto/lex open
npm install -D typescript @types/node
npx tsc --init
```

You'll also need our `lex` CLI tool installed globally, and the `app.bsky.actor.profile` Lexicon to fetch the user's profile:
...
The `NodeOAuthClient` manages the OAuth flow. `buildAtprotoLoopbackClientMetadata` configures it for local development — AT Protocol has a special [loopback carveout](/specs/oauth#localhost-client-development) so you don't need a public URL.
...
The `Client` from `@atproto/lex` accepts the OAuth session directly — the same way it accepts a `PasswordSession` in the [bot tutorial](/guides/bot-tutorial). From here you can `.get()`, `.list()`, `.create()`, and `.call()` any installed Lexicon.
