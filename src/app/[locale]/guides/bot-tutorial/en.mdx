...
This tutorial involves building a TypeScript application from scratch. You should have a fundamental understanding of TypeScript and Node.js.
...
If you aren't using your own account, you should [create an account](https://bsky.app/) for your bot. You should also generate an [app password](/guides/sdk-auth) for the bot.

## Part 1: Lexicons 

[Lexicons](/guides/lexicon) define ATProto records. Your bot will create Bluesky posts, using the [app.bsky.feed.post](https://pdsls.dev/at://did:plc:ewvi7nxzyoun6zhxrhs64oiz/app.bsky.feed.post/3juc4thkx452i#schema) Lexicon. You can use `lex` to download and build this Lexicon into your project:
...
Finally, add another code block using the `cron` package to schedule your bot to post every three hours:
...
Now you can replace the `makePost` function in your bot with new logic that uses the AI SDK to generate replies:
...
async function checkMentions() {
  const notifications = await client.list(app.bsky.notification.listNotifications, {
    limit: 20,
  })
  ...
  for (const notif of notifications.notifications) {
    ...
    const post = await client.get(app.bsky.feed.getPostThread, {
      uri: notif.uri,
    })
...
const job = new CronJob('*/5 * * * *', checkMentions)
job.start()

console.log('Agent is running...')
```

There are a few new concepts here. You'll recognize the `app.bsky.post.logic`, and now you'll also implement `app.bsky.notification.listNotifications` and `app.bsky.feed.getPostThread`. The first Lexicon is used to get a list of notifications for, and the second fetches the full thread for a given post URI.
