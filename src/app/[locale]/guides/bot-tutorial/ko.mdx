...
## 파트 1: 어휘집
[어휘집](/guides/lexicon)은 Atproto 레코드를 정의합니다. 봇은 [app.bsky.feed.post](https://lexicon.garden/lexicon/did:plc:4v4y5r3lwsbtmsxhile2ljac/app.bsky.feed.post) 어휘를 사용하여 Bluesky 게시물을 생성합니다. `lex`를 사용하여 이 어휘집을 다운로드하고 프로젝트에 빌드할 수 있습니다:
...
봇에 필요한 코드는 여기까지입니다! 다음으로 봇으로 라벨을 지정합니다.

## 3부: 봇으로 라벨 지정

모범 사례로서, 봇 계정은 프로필에 셀프 라벨을 추가하여 자신을 식별해야 합니다. 이를 통해 사용자와 모더레이션 도구가 자동화된 계정을 인식할 수 있습니다. `src/index.ts`에 다음 함수를 추가하세요:

<S08a />

봇을 처음 설정할 때 `labelAsBot(session)`을 한 번 호출하세요. 이 작업은 한 번만 수행하면 됩니다. 라벨은 프로필에 영구적으로 저장됩니다. cron 작업을 시작하기 전에 `main()`에서 호출한 다음, 첫 실행 후 해당 호출을 제거할 수 있습니다.
