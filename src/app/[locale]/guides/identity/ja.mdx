...
<Heading level={2} id="did-methods">DID メソッド</Heading>
...
プロジェクト開始時点で、DID 方式のいずれも当社の基準を完全に満たしていませんでした。 **そのため、私たちは[did-web](https://w3c-ccg.github.io/did-method-web/)と、私たちが作成した[DID PLC](https://github.com/bluesky-social/did-method-plc)という新規の方法をサポートすることを選択しました。**
...
<Heading level={3} id="example-hosting-service">例: ホスティング サービス</Heading>
...
<Heading level={3} id="example-hosting-service-with-different-domain-name">例: 別のドメイン名を持つホスティング サービス</Heading>
...
<Heading level={3} id="example-self-hosting">例: セルフ ホスト</Heading>
