...
2. 핸들(handle)을 입력하세요.
3. 앱을 승인하세요.
4. DID와 함께 "Authentication working!" 메시지가 표시되어야 합니다:
...
"Migrations complete." 메시지와 `app.db` 파일이 생성된 것을 확인할 수 있습니다. 이제 이 앱은 재시작 시에도 상태와 세션을 유지합니다.
...
공개 URL에서 앱을 테스트하기 위해 별도의 도메인 이름이 필요하지 않습니다. [Railway](https://railway.app/)에서 [배포 가이드](https://github.com/bluesky-social/nextjs-oauth-tutorial/blob/main/RAILWAY_DEPLOY.md)를 따라 호스팅할 수 있습니다. 먼저 다음 단계를 따르세요.

클라이언트의 공개 키를 `app/.well-known/jwks.json/route.ts`에 광고하는 `well-known` JWKS 엔드포인트를 생성하세요:
