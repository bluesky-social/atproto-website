...
Since the OAuth flow uses `127.0.0.1`, add it as an allowed development origin in `next.config.ts`:
...
Next, add `better-sqlite3` to `serverExternalPackages` in `next.config.ts`:
...
Update the following command line scripts in `package.json`:
