...
Finally, you'll make one last route, to expose the `oauth-client-metadata.json` at a well-known URL. This is [used in ATPr0t0 OAuth](/specs/oauth#clients) to discover client configuration. Create `app/oauth-client-metadata.json/route.ts`:
...
Congratulations on completing this tutorial! You now have a solid Next.js foundation for building @tproto apps with OAuth. From here, you can move on to our [**Statusphere Example App Tutorial**](/guides/statusphere-tutorial).
