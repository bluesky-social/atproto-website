...
<Heading level={2} id="part-2-implementing-o-auth">パート2：OAuthの実装</Heading>
...
これで[クライアントメタデータドキュメント](/specs/oauth#client-id-metadata-document)を配信できます。

<Note>
`dpop_bound_access_tokens: true`設定は、アクセストークンを暗号的に特定のクライアントに紐付ける[DPoP（Demonstrating Proof of Possession）](/specs/oauth#demonstating-proof-of-possession-d-po-p)を有効にします。これにより、存続期間の長いセッションがトークンの盗難から保護されます。SDKはすべてのDPoP操作を自動的に処理します。proofの生成、サーバーから提供されるnonceの管理、nonceローテーション時のリクエストの再試行などです。SDKなしでOAuthを実装している場合（サイドカーサービスなど）、実装の詳細については[OAuthパターンガイド](/guides/oauth-patterns#understanding-d-po-p)を参照してください。
</Note>
...
デフォルトでは、アプリは`atproto`スコープのみを要求します。`atproto`スコープは必須で、`atproto` IDの基本認証を提供しますが、特権情報へのアクセスやユーザーの代理操作は許可しません。詳細は[権限セットガイド](/guides/permission-sets)を参照してください。
