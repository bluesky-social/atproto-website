...
PDSは多くのATエンドポイントを実装しており、デフォルトでリクエストを他のサービスにプロキシします。キャッシュやCDNなどは通常、[AppView](#app-views)レイヤーで実装されます。 ATスタックの他の大半のコンポーネントは、PDSに直接負荷をかけないように設計されているため、ユーザーデータのホスティングが経済的に維持できます。
...
デフォルトでは、新しいatproto IDは`did:plc`ドキュメントとして作成されます。

[did:plc](https://web.plc.directory/)は自己認証型の[分散型識別子](https://en.wikipedia.org/wiki/Decentralized_identifier)で、強い一貫性があり、復旧可能で、鍵のローテーションを可能にします。各IDは、前のオペレーションを連鎖的に参照する署名付きオペレーションの系列で構成され、ID作成オペレーションまで遡ります。任意の観測者はオペレーションの連鎖を検証することで、IDの現在の状態を検証できます。
...
- https://github.com/bluesky-social/atproto リポジトリには、[PDS](https://github.com/bluesky-social/atproto/tree/main/packages/pds)のコードとTypeScript SDKを含む、TypeScriptツールの大半が含まれています。[atcute](https://github.com/mary-ext/atcute)コミュニティプロジェクトもまた、別のTypeScriptパッケージを提供します。
