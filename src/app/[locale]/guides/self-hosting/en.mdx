...
Hosting your own Relay is possible, though bandwidth intensive, as generally relays operated in full-network mode, where they replicate data from all PDSes and repositories on the AT Protocol network. Previously operating a relay was also resource intensive due to data storage requirements, as of Sync 1.1, the data storage requirements are no longer an issue, as relays are not required to be archival.

There are a few different relay implementations:
- [Reference Implementation](https://github.com/bluesky-social/indigo/blob/main/cmd/relay/README.md)
