...
An identity on the atproto network can have multiple accounts on different PDSs, but only one account can be the identity's active account signing new records. To create a PDS account with an existing identity, it is necessary to prove control of the identity.
...
For an independently-controlled identity (eg, `did:web`, or a `did:plc` with the old PDS offline or uncooperative), this may involve updating the identity to include a self-controlled atproto signing key, and generating the service auth token offline. This is often referred to as an "Adversarial Migration" with the steps outlined in [Adversarial ATProto PDS Migration](https://www.da.vidbuchanan.co.uk/blog/adversarial-pds-migration.html).

The service auth token is provided as a `Bearer` `Authorization` header along with the existing DID when creating the account with `com.atproto.server.createAccount` (or an equivalent interface/API) on the new PDS to prove then identity is requesting for a new account to be created on the target PDS.
...
Some PDSs may lock down the ability to create new accounts, this can be checked at `com.atproto.server.describeServer` on the target PDS. This is usually locked by an invite code (`inviteCodeRequired`), or solving a challenge (`phoneVerificationRequired`) and submitting it with the create account request. A combination of these may used to lock migrations to the target PDS unless it was the origin of your identity, like in the case of `https://bsky.social`.
...
If there was previously an account on the target PDS, or a migration was started and never completed, you can call `com.atproto.server.checkAccountStatus` to get the current repo revision of that account on the target PDS and use the repo's revision with the `since` param on `com.atproto.repo.getRepo` from the source PDS to import a smaller diff of the repository containing only the new data since the last import. It also does not harm anything to re-import the repo multiple times from source to target, and is recommended to do so on migration retries to ensure all data is imported successfully from the source PDS. The PDS handles the diff of the import uploaded and what it has stored.

Larger repo exports may fail on import and give unexpected errors. This is not always related to memory exhaustion on the target PDS, but it is often a good first check. This also shows as a `CORS` error in the browser console if a migration runs in the web browser due to the unexpected disconnect.

### Blobs

Blobs (media files) are download and re-uploaded one by one. They should not be uploaded to the new PDS until the repository has been imported and fully indexed, so that blobs can be linked to the records that refer to them, and won’t be garbage collected. A full list of relevant blobs (by CID) can be fetched either from the old PDS (`com.atproto.sync.listBlobs`), or the current list of "missing" blobs can be checked on the new PDS (`com.atproto.repo.listMissingBlobs`). It is recommended if the target PDS already has an account at the start of the migration to use `com.atproto.repo.listMissingBlobs` after a repo import so you are not reuploading any blobs that the target PDS may already have.

If some blobs can not be found, the migration process can continue, and any recovered blobs can be uploaded later (if the blob CIDs match exactly). When accessing blobs from a "deactivated" account the request must be authenicated using the account from that PDS, other wise you cannot access them. 

Most PDSs also implement a rate limit on blob uploads. For the refernce PDS this is 1,000 uploads per 24 hours, per an IP address. This can be bypassed on the refernce PDS by adding IPs to the `PDS_RATE_LIMIT_BYPASS_IPS` environment variable, or by `PDS_RATE_LIMIT_BYPASS_KEY` that matches teh same password on the `x-ratelimit-bypass` header up the upload request. This should only be done for trusted sources, since it is a bypass for all rate limits.
...
For users who are able to securely manage a private cryptographic keypair (eg, store in a password manager or digital wallet), it is recommended to include a self-controlled PLC rotation key (public key) in the PLC operation. These are refered to as Rotation keys.
