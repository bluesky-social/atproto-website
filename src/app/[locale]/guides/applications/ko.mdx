...
<Heading level={2} id="step-1-expressjs-app-setup">1단계. ExpressJS 앱 시작하기</Heading>
...
<Heading level={2} id="step-2-login-with-oauth">2단계. OAuth를 통한 로그인</Heading>

사용자가 앱에 로그인하면, 개인의 `at://` 레포지토리에 대한 읽기 및 쓰기 권한을 부여하게 됩니다. 이를 통해 상태 JSON 레코드를 작성할 수 있습니다.

OAuth ([사양](https://github.com/bluesky-social/proposals/tree/main/0004-oauth))를 사용하며, 대부분의 OAuth 흐름은 [@atproto/oauth-client-node](https://github.com/bluesky-social/atproto/tree/main/packages/oauth/oauth-client-node) 라이브러리가 처리합니다. 기본적인 흐름은 다음과 같습니다:
...
로그인 페이지는 사용자의 "핸들" (예: `alice.bsky.social` 또는 도메인 이름)을 입력받습니다.
...
<form action="/login" method="post" class="login-form">
  <input
    type="text"
    name="handle"
    placeholder="핸들 입력 (예: alice.bsky.social)"
    required
  />
...
<Heading level={2} id="step-3-fetching-the-users-profile">3단계. 사용자 프로필 가져오기</Heading>

사용자에 대해 좀 더 알아볼까요? [Bluesky](https://bsky.app)에서는 사용자가 "프로필" 레코드를 게시하며, 다음과 같은 구조를 가집니다:
...
이 레코드는 [atproto-browser.vercel.app](https://atproto-browser.vercel.app)에서 직접 확인할 수 있습니다. 예를 들어, [@bsky.app의 프로필](https://atproto-browser.vercel.app/at?u=at://did:plc:z72i7hdynmk6r22z27h6tvur/app.bsky.actor.profile/self)을 볼 수 있습니다.

사용자의 OAuth 세션과 연결된 [Agent](https://github.com/bluesky-social/atproto/tree/main/packages/api)를 사용해 이 레코드를 가져옵니다.

```typescript
await agent.com.atproto.repo.getRecord({
  repo: agent.assertDid,                // 사용자 DID
  collection: 'app.bsky.actor.profile', // 컬렉션 이름
  rkey: 'self',                         // 레코드 키
})
```
...
router.get(
  ...
  handler(async (req, res) => {
    ...
    const { data: profileRecord } = await agent.com.atproto.repo.getRecord({
      ...
      collection: 'app.bsky.actor.profile', // Bluesky 프로필 레코드
      rkey: 'self',                         // 레코드 키
    })
...
<Heading level={2} id="step-4-reading-writing-records">4단계. 레코드 읽기 및 쓰기</Heading>
...
<Heading level={2} id="step-5-creating-a-custom-status-schema">5단계. 커스텀 "상태" 스키마 생성</Heading>

레포지토리의 컬렉션은 타입이 지정되어 있으며, 정해진 스키마를 따릅니다. `app.bsky.actor.profile` 타입 정의는 [여기](https://github.com/bluesky-social/atproto/blob/main/lexicons/app/bsky/actor/profile.json)에서 확인할 수 있습니다.
...
> <Heading level={3} id="why-create-a-schema">왜 스키마를 만들까요?</Heading>
>
...
<Heading level={2} id="step-6-listening-to-the-firehose">6단계. 이벤트 스트림(firehose) 구독</Heading>
...
<Heading level={2} id="step-7-listing-the-latest-statuses">7단계. 최신 상태 목록 표시</Heading>
...
<Heading level={2} id="step-8-optimistic-updates">8단계. 낙관적 업데이트 (Optimistic Updates)</Heading>
...
<Heading level={2} id="thinking-in-at-proto">AT Protocol 사고방식</Heading>
...
이와 같은 방식으로 Bluesky 소셜 앱을 포함한 모든 Atmosphere 애플리케이션이 동작합니다.

<Heading level={2} id="next-steps">다음 단계</Heading>

배운 내용을 연습해보고 싶다면, 다음 도전 과제들을 시도해보세요:

- 모든 사용자의 프로필 레코드를 동기화하여 핸들 대신 표시 이름을 보여주기
- 각 상태 이모지의 사용 횟수를 집계하여 총합 표시하기
- 인증된 사용자의 `app.bsky.graph.follow` 정보를 가져와 팔로잉 상태 표시하기
