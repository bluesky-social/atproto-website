...
- Listen to the network firehose or jetstream for new data {{className: 'lead'}}
...
## Step 6. Listening to the firehose/jetstream
### Firehose
So far, we have:
...
### Jetstream
A [Jetstream](https://docs.bsky.app/blog/jetstream) instance is downstream of the Firehose, and let's us listen to events on the network through websockets, in JSON, without the cryptographic benefits the Firehose has. We will still filter to `xyz.statusphere.status` records, but since our application is small and we are listening to a trusted jetstream operator, it is a good use case for our app.

```typescript
/** src/jetstream-ingester.ts **/
import * as Status from '#/lexicon/types/xyz/statusphere/status'
// ...
...
```
Now we can write these statuses into our database as they arrive from the jetstream:
...
Now in our startup file, just swap ingesters to start listening to the Jetstream
```ts
/** src/index.ts **/
