...
リポジトリは通常の Web アプリです。1999 年のように HTML サーバー側をレンダリングしています。また、[Kysley](https://kysely.dev/) で管理している SQLite データベースもあります。

開始スタック:

- Typescript
- NodeJS Web サーバー ([express](https://expressjs.com/))
- SQLite データベース ([Kysley](https://kysely.dev/))
...
このコードは、`firehose.ts` で行っていることとほとんど同じであることがわかります。
