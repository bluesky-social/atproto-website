import { Card, CardParagraph, CardDetails, CardTitle } from '@/components/Card'
...
Atproto is like a souped-up RSS. Users publish JSON records into repositories. The changestreams of those records then sync across the network to drive applications.

Here are the core concepts:

- **User repos.** The public per-user databases.
- **User handles.** Aka usernames, which are DNS records. Our account is `@atproto.com`.
- **User DIDs.** The permanent IDs of users.

Data gets published in a couple of forms:

- **Records**. JSON, categorized into collections.
- **Blobs**. The images and videos.

Everything is described using schemas:

- **Lexicon**. The schema language.
- **LexRPC (XRPC).** It's HTTP but with the routes defined by Lexicons.
