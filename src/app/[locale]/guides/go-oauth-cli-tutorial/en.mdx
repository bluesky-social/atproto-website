...
In this tutorial, you'll build a minimal Go command-line tool that authenticates a user with [ATProto OAuth](/guides/auth) and fetches their session info. The entire app fits in a single file.
...
Create a new Go module and install the indigo SDK:
...
That's it — the indigo SDK includes everything you need for OAuth, including an in-memory session store for development.
...
`NewLocalhostConfig` sets up a [loopback client](/specs/oauth#localhost-client-development) — AT Protocol has a special carveout for localhost so you don't need a public URL. `NewMemStore` provides in-memory session storage for development.
...
- **Sessions are in-memory.** Every run requires re-authorization. The Cookbook example above shows how to persist sessions with SQLite.
