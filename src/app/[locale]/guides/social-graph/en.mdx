...
When building new experiences on atproto, it's up to you to decide how much to reuse the existing social graph from popular Lexicons on the network. For example, some apps might choose to reuse "follow" or "block" relationships from `app.bsky.graph` — perhaps by integration Microcosm — while others might implement their own. You could also reuse the media blobs from existing posts to build an image board or video sharing app to bootstrap with existing content. This has implications for how much data you need to [backfill](/guides/backfilling), and how much hosting infrastructure you may need to drive an [app view](/guides/the-at-stack#app-views).
