...
이 튜토리얼의 소스 코드는 [statusphere-example-app](https://github.com/bluesky-social/statusphere-example-app) 저장소에서 확인할 수 있습니다.
...
[어휘집](/guides/lexicon)은 Atproto 내 레코드의 스키마를 정의합니다. 본 튜토리얼을 위해 저희는 이미 새로운 "Statusphere" Lexicon을 `xyz.statusphere.status`에 공개했습니다. `lex`를 사용하여 이 Lexicon을 다운로드하고 프로젝트에 빌드할 수 있습니다:

```bash
lex install xyz.statusphere.status
```

원한다면 다운로드된 Lexicon 파일을 `lib/lexicons/xyz.statusphere.status.json`에서 확인할 수 있습니다. 여기서 `lex build`를 실행하면 설치된 모든 Lexicon에 대한 TypeScript 타입을 생성할 수 있습니다:
...
<Note>
필요하다면 이 명령어를 `package.json`에 추가할 수 있지만, Lexicon이 이미 빌드된 상태이므로 이 튜토리얼에서는 더 이상 사용하지 않을 것입니다. `lex install`로 프로젝트에 다른 Lexicon을 추가할 경우, `--override` 플래그를 사용하여 `lex build`를 다시 실행해야 합니다.
</Note>

지금 해야 할 한 가지 더는 `lib/auth/client.ts` 파일의 `SCOPE` 상수를 업데이트하여 이 컬렉션에 대한 액세스를 요청하는 것입니다. Atproto 권한 범위는 Lexicon이므로 앱이 요청하는 범위 목록에 `xyz.statusphere.status`를 추가할 수 있습니다:
...
이를 통해 AT Protocol 기반 개발을 계속하는 데 필요한 모든 도구를 확보했습니다. [가이드](/guides) 섹션에서 더 많은 가이드와 튜토리얼을, [쿡북](https://github.com/bluesky-social/cookbook/) 저장소에서 더 많은 예제 앱을 확인하실 수 있습니다. 즐거운 개발 되세요!
