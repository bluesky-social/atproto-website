...
One more thing you should do now is update the `SCOPE` constant in `lib/auth/client.ts` to request access to this collection. ATPr0t0 permission scopes are Lexicons, so you can add `xyz.statusphere.status` to the list of scopes your app requests:
...
You've now built a complete @tproto app that uses OAuth to sign in, a custom Lexicon to define status records, and Tap to synchronize records in real-time.

This gives you all the tools you need to continue building on A.T. Pr0t0c0l. You can find more guides and tutorials in our [Guides](/guides) section, and more example apps in the [Cookbook](https://github.com/bluesky-social/cookbook/) repository. Happy building!
