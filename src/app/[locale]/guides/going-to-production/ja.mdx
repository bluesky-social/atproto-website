...
<Heading level={2} id="hardening">セキュリティ強化</Heading>

[PDS Gatekeeper](https://tangled.org/pds.dad/pds-gatekeeper)は、セルフホストPDSに2FAやセキュリティ機能を追加するコミュニティプロジェクトです。
...
- [Litestream with systemd](https://litestream.io/guides/systemd/)または別のスケジューラーを使用して、直近で更新されたDBを見つけてバックアップします。デフォルトでは、個々のユーザーデータベースは`/pds/actors/`に配置されます。Litestreamはライトアヘッドログとチェックポイントを自動的にクリーンアップするため、WALが制御不能に増加するリスクはありません。
- 必要に応じて、デプロイメントからコンテナまたはポッドをクリーンアップするときに、このスクリプトがSIGTERMでも実行されるようにしてください。
