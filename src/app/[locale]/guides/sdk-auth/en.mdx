...
<TabGroup defaultValue="ts">
  ...
  <TabPanels>
    ...
    <TabPanel value="go">
    Start by importing the `atclient` library and calling `LoginWithPasswordHost()`:
...
<TabGroup defaultValue="ts">
  ...
  <TabPanels>
    ...
    <TabPanel value="go">
    The `client` returned by `LoginWithPasswordHost()` is already authenticated and ready to make API calls:

    ```go
    // The client handles auth automatically, including token refresh
    fmt.Printf("Logged in as: %s\n", client.AccountDID)

    // Now you can make authenticated API calls with the client
    ```
    </TabPanel>
  </TabPanels>
</TabGroup>
