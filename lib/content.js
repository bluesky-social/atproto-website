...
export async function getNavigationWithDesc(currentHref) {
  ...
  for (const item of [...data.guides, ...data.specs]) {
...
function admonitions() {
  const classes = {
    ...
    warn: 'admonition admonition--warn',
  ...
  return (tree) => {
    visit(tree, (node) => {
      if (
        ...
        if (!['note', 'tip', 'warn', 'danger'].includes(node.name)) return

        const data = node.data || (node.data = {})
        const tagName = node.type === 'textDirective' ? 'span' : 'div'

        data.hName = tagName
        data.hProperties = { className: classes[node.name] }
      }
    })
  }
}
