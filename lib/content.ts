...
export async function getNavigationWithDesc() {
  const data = await getNavigation();
  for (const item of [...data.guides, ...data.specs, ...data.lexicons]) {
    const file = await getFile(
      ...(item.href.split("/").filter(Boolean) as [string, string])
    );
...
export async function getFile(folder: string, id: string) {
